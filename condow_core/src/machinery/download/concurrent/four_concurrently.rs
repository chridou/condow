...
pin_project! {
    /// Downloads pats with a maximum concurrency of 4.
    ///
            ...
            + 'static,
    {
        ...
        let active_streams = match (
            ...
            (Some(first), Some(second), Some(third), None) => {
                ...
                ActiveStreams::LastThreeConcurrently {
                    left,
                    middle,
                    right,
                }
            ...
            (Some(first), Some(second), Some(third), Some(fourth)) => {
                ...
                let mid_left = PartChunksStream::new(
                    &get_part_stream,
                    second,
                    probe.clone(),
                    download_span_guard.span(),
                );
                let mid_right = PartChunksStream::new(
                    &get_part_stream,
                    third,
                    probe.clone(),
                    download_span_guard.span(),
                ...
                ActiveStreams::FourConcurrently {
                    left,
                    mid_left,
                    mid_right,
                    right,
    ...
    fn poll_next(
        ...
        match active_streams {
            ...
            ActiveStreams::LastThreeConcurrently {
                ...
                let (poll_result, next_state) = match poll_last_three(left, middle, right, cx) {
                    Ok(ok) => ok,
                    Err(err) => {
                        this.baggage
                            .probe
...
/// poll "left biased" until there are only 3 parts left
///
/// There are 4 or more parts left to download.
///
...
fn poll_four<P: Probe + Clone>(
    ...
    match mid_left.poll_next_unpin(cx) {
        Poll::Ready(Some(Ok(chunk))) => {
            return Ok((
                Poll::Ready(Some(Ok(chunk))),
                ActiveStreams::FourConcurrently {
    ...
    match mid_right.poll_next_unpin(cx) {
        Poll::Ready(Some(Ok(chunk))) => {
            return Ok((
                Poll::Ready(Some(Ok(chunk))),
                ActiveStreams::FourConcurrently {
...
mod tests {
    ...
    use super::FourPartsConcurrently;
    ...
    async fn empty() {
        ...
        let stream = FourPartsConcurrently::from_client(
            client.clone(),
            IgnoreLocation,
            part_requests,
            (),
    ...
    async fn ten_parts() {
        let client = ClientRetryWrapper::new(TestCondowClient::new().max_jitter_ms(5), None);
        let part_requests = PartRequestIterator::new(0..=99, 10);
