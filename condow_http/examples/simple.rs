...
async fn main() -> Result<(), AnyError> {
    let config = Config::default().buffer_size(1).part_size_bytes(1000).always_get_size(true);
    ...
    let res = condow.download(String::from(url), ..).await.unwrap().into_vec().await.unwrap();
